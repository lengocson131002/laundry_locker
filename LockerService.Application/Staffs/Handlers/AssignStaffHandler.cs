...
public class AssignStaffHandler : IRequestHandler<AssignStaffCommand, StatusResponse>
{
    ...
    public AssignStaffHandler(IMapper mapper, IUnitOfWork unitOfWork, ILogger<AssignStaffHandler> logger,
    ...
    public async Task<StatusResponse> Handle(AssignStaffCommand request, CancellationToken cancellationToken)
    {
        ...
        var accountLockerQuery =
            await _unitOfWork.AccountLockerRepository.GetAsync(al =>
                Equals(al.AccountId, request.Id) && Equals(al.LockerId, request.LockerId));
